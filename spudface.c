...
# define GETVRT(V,V0,V1,V2) ((V)==(0) ? V0 : ((V)==(1) ? V1 : ( V2 )))
  ...
  VEC v, vcp;
  double lVcp;
    vsub_c( spudf->Rxyz + (3 * (*vrtPtr)), vtx0, v);      /* maxIv to *vrtPtr */
    vcrss_c( vtx0, v, vcp);              /* cross product of radius & segment */
    lVcp = vnorm_c( vcp);                          /* length of cross product */
   ...
   *       to maxIv radius i.e. has greatest vdot_c between normal and vtx0
  ...
  lclPlts[0] = 
    (vdot_c(vtx0,luvnorms+(3*segPlt->_ip0))>vdot_c(vtx0,luvnorms+(3*segPlt->_ip1)))
  ...
  VEC otherVec;
    ...
    for ( idxBaseVrt=0; idxBaseVrt<3; ++idxBaseVrt) {
      ...
      *(endPltPtr++) = ipAdj;
       ...
       */
      vsub_c(vtx1,vtx0,otherVec);
      dotxdot = vdot_c( adjNorm, otherVec) * vdot_c( baseNorm, otherVec);

      ovlen = vnorm_c(otherVec);
      if ( ovlen > 0.0 ) {
        dotxdot /= (ovlen*ovlen);                         /* unitize otherVec */
      } else {   /* ovlen is 0; otherVec is null; vtx0 & vtx1 are the same pt */
        vminus_c( baseNorm, adjNorm);       /* adj norm is opposite base norm */
      ...
      *
      * - if adjNorm is NOT ok, vminus_c it
      */
     if ( dotxdot < 0.0) continue;
     if ( dotxdot < 1e-12) if ( vdot_c(adjNorm,baseNorm) > 0.0) continue;

     vminus_c( adjNorm, adjNorm);                           /* fix the normal */
...
#      define PVEC( T, P) fprintf( stderr, "%s ( %lg %lg %lg )\n", T \
       ...
       fprintf( stderr, "Base vdot_c(R0,uvn) = %lg\n"
              , vdot_c( spudf->Rxyz + (3 * spudf->faceapices[*pltPtr])
                    , luvnorms + (3 * *pltPtr))  );
       fprintf( stderr, "Adj  vdot_c(R0,uvn) = %lg\n"
              , vdot_c( spudf->Rxyz + (3 * spudf->faceapices[ipAdj])
                    , luvnorms + (3 * ipAdj)) );
...
VEC vg, v2g;
  ...
  for ( ip=0; ip<spudf->nface; ++ip, (uvn+=3), (mp+=3), ++r2) {
    ...
    vsub_c( vtx1, vtx0, seg10);           /* vector from vertex 0 to vertex 1 */
    vsub_c( vtx2, vtx0, seg20);           /* vector from vertex 0 to vertex 2 */
    vcrss_c( seg10,seg20,uvn); /* cross product of segments is normal to face */
    vl = 1.0 / vnorm_c(uvn);                          /* normalization factor */
    /* ensure normal faces outward
     * - dot prod of normal w/radial vector to any vertex should be > 0
     */
    if ( vdot_c( uvn,vtx0) < 0.0) { vscl_c( -vl, uvn, uvn); }
    else { vscl_c( vl, uvn, uvn); }
     ...
     */
    vsub_c( vtx2, vtx1, seg21);
    l10 = vdot_c(seg10,seg10);
    l20 = vdot_c(seg20,seg20);
    l21 = vdot_c(seg21,seg21);
...
#   define VPSW( V0, V1) vtmp=V0; V0=V1; V1=vtmp
    ...
    if ( l10>=l20) {
      ...
      if ( l10>=l21) { CBA2(l10,l21,l20); vminus_c(seg10,seg01); }
      /* l21 is largest, swap vertices 0 & 2, copy seg21 */
      else { CBA2(l21,l10,l20); VPSW(vtx2,vtx0); vequ_c(seg21,seg01); }
    } else {
      /* l20 is largest, swap vertices 1 & 2, invert seg20 */
      if ( l20>=l21) { CBA2(l20,l21,l10); VPSW(vtx2,vtx1); vminus_c(seg20,seg01); }
      /* l21, same as above */
      else { CBA2(l21,l10,l20); VPSW(vtx2,vtx0); vequ_c(seg21,seg01); }
    }

    vsub_c( vtx0, vtx1, seg01);

    /* initially put midpoint at middle of longest side */

    vlcom_c( 0.5, seg01, 1., vtx1, mp);
    *r2 = c2 / 4;
    ...
    if ( (a2+b2) > c2) {
      ...
      eod = (d2+(g*g)-(c2/4)) / (2*d2); /* dist from V01 midpt to facet midpt */
                                                        /* as a fraction of d */
      vlcom_c( cb/c, seg01, 1., vtx1, vg);  /* vg = projection of V2 onto V01 */
      vsub_c( vtx2, vg, v2g);            /* v2g = vector from vg on V01 to V2 */
      vlcom_c( eod, v2g, 1., mp, mp);                         /* adjust midpt */
...
# define ICOPTR(S) (spudf->Rxyz + (3*(S)))
  ...
  ksofar = vnorm_c(sc)* 1.0e10; /* no intersect'n yet, set > any intersection */
  ...
  for ( ip=0; ip<spudf->nface; ++ip, ++fa, ++foidx, ++r2, (mp+=3), (uvn+=3)) {
    while (1) {         /* not really a loop, use here to force break to exit */
      bsdn = vdot_c( ubs, uvn);                 /* boresight dot plate normal */
      if ( bsdn >= 0.0) break;              /* keep plate emission angle < 90 */
      vsub_c( mp, sc, mpmsc);                               /* sc to midpoint */

#     ifndef _ALTERNATE_SPUDF_INT_
      b = vdot_c( ubs, mpmsc);   /* SC to closest-point-on-boresight-to-midpt */
      c2 = vdot_c( mpmsc, mpmsc);                      /* SC to midpt squared */
      a2 = c2 - (b*b);                   /* min boresight distance from midpt */
      if ( a2 > *r2) break;     /* boresight must pass within radius of midpt */
      if ( (k=vdot_c(mpmsc,uvn)/bsdn) < 0.0) break;
      if ( k >= ksofar) break;  /* must intersect before current intersection */
      vlcom_c(k,ubs, 1.,sc, scpkbs); /* intersection, k * boresight + sc, ABF */
#     else
      k=vdot_c(mpmsc,uvn)/bsdn;                  /* k = range to intersection */
      vlcom_c(k,ubs, 1.,sc, scpkbs); /* intersection, k * boresight + sc, ABF */
      vsub_c( mp, scpkbs, mpmi);                  /* intersection to midpoint */
      if ( vdot_c(mpmi,mpmi) > *r2) break; /* ... range must pass close to mp */
...
#     endif
      ...
      vsub_c( vtx0, scpkbs, v0);     /* vectors from intersection to vertices */
      vsub_c( vtx1, scpkbs, v1);
      vsub_c( vtx2, scpkbs, v2);
      vcrss_c( v0, v1, v0xv1);      /* cross products of sequence of vertices */
      vcrss_c( v1, v2, v1xv2);
      if ( (dot012=vdot_c(v0xv1,v1xv2))<0.0) break; /* cp dot cp should be >0 */
      vcrss_c( v2, v0, v2xv0);                      /* if intersection is     */
      if ( (dot120=vdot_c(v1xv2,v2xv0))<0.0) break; /* inside triangle        */
      if ( (dot201=vdot_c(v2xv0,v0xv1)) < 0.0) break;
      ...
      double lv01, lv12, lv20;            /* in line & beyond a segment       */
        if ( dot012 == 0.0 && dot201 == 0.0) {                /* - vtx0->vtx1 */
          vsub_c( v0, v1, v01); lv01 = vdot_c(v01,v01);   /* - segment length */
          if (lv01 < vdot_c(v0,v0) || lv01 < vdot_c(v1,v1))break; /* - beyond */
        } else if ( dot120 == 0.0 && dot012 == 0.0) {         /* - vtx1->vtx2 */
          vsub_c( v1, v2, v12); lv12 = vdot_c(v12,v12);   /* - segment length */
          if (lv12<vdot_c(v1,v1) || lv12<vdot_c(v2,v2)) break;    /* - beyond */
        } else if ( dot201 == 0.0 && dot120 == 0.0) {         /* - vtx2->vtx0 */
          vsub_c( v2, v0, v20); lv20 = vdot_c(v20,v20);   /* - segment length */
          if (lv20<vdot_c(v2,v2) || lv20<vdot_c(v0,v0)) break;    /* - beyond */
        }
      }
