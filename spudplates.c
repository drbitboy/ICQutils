...
/*********************************************************************/
/* renumber plate model's vertices by increasing latitude so fps's plate 
...
# define MALLOC( PTR, CAST, COUNT) \
  ...
  switch ( scanstat0 ) {
  case 2:
    pDumy = fgets( str, 255, f);          /* scanstat0==2:  skip first line (NV NP) */
                                      /* ***N.B. DROP THROUGH TO scanstat0==3 */
  /*********************/
  case 3:                                                  /* read 'vx vy vz' */
    for ( iv=0; iv<spudf->nv; ++iv) {
      pDumy = fgets( str, 255, f);
...
#     define DOMAX \
      vsub_c( spudf->Rxyz+(3*iv), spudf->Rxyz+(3*spudf->oe[iseg-1]), tmpvec); \
      tmpmax2 = vdot_c( tmpvec, tmpvec); \
