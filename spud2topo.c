/* Convert Thomas SPUD model (R, Latitude, Longitude) to TOPO format
 *
 * Usage:
 *
 *  [VIEW=spud.shape] ./spud2topo
 *
...
double *vPtr;
  ...
  vadd_c( vReturn, vPtr, vReturn);
  vPtr = spudf->Rxyz + (3 * spudf->oe[spudf->faceoeidx[pltNum] + 1]);
  vadd_c( vReturn, vPtr, vReturn);
  vReturnLen = 1.0 / 3.0;
  vscl_c( vReturnLen, vReturn, vReturn);
  vReturnLen = vnorm_c( vReturn);
  return vReturnLen;
}
...
double maxR;
  ...
  long pltNumIntersected;
    ...
    vscl_c( -cfRad, cf, uvToOrigin);
    vscl_c( scalToMaxR, cf, cf);
