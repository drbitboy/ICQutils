...
# endif
  ...
  if ( range == 0.0) { 
    vminus_c( MTXR[1], viewabf);          /* view from infinity, use unit vec */

  } else {                   /* **************** PERSPECTIVE***************** */
    for ( i=0;i<3;++i) {
      scCam[i] = vdot_c(MTXR[i],scAbf);
    }
    vscl_c( 1.0, MTXR[2], viewUpAbf);          /* camera up, unit vector, ABF */
  }
  ...
  VEC scToXyz; \
    ...
    for (S=0; S < numvert; S++, basexyz += 3, spudvxyz += 3) { \
      ...
      double *mtxr;            /* unless doing hidden gridpt (istep=1) */ \
        mtxr = MTXR[i]; \
        spudvxyz[i] = vdot_c(mtxr,basexyz); \
      } \
      \
...
/**/
    ROT
      ...
      if ( range == 0.0) *(lv0++) = *spudvxyz;       /* no perspective: use X */
        ...
        vsub_c( spudvxyz, scCam, scToXyz);       /* perspective:  use azimuth */
...
#                  define PI_SPEC (M_PI + 1.0)
        ...
        *(lsinalt0++) = scToXyz[2] / vnorm_c(scToXyz); 
/**/
      }
    } /* ROT */
...
/* #define CALCXYZVEC(V,I) vequ_c(V,scToXyz[I]); scToXyz[I][1] += range */
#define CALCXYZVEC(V,I) vequ_c(V,scToXyz[I]); vsub_c( V, scCam, scToXyz[I]);
...
/**/
  ...
  for ( ip=0; ip<numface; ++ip) {
     ...
     */
    vsub_c( scAbf, spudf->Rxyz+(iv3[0]*3), xyzToSc);
    if ( vdot_c( xyzToSc, spudf->uvnorms+(3*ip)) <= 0.0) continue;
...
/**/
    ...
    if ( vdot_c( scToXyz[0], pltNorm) > 0.0) { vminus_c(pltNorm, pltNorm); } 
    ...
    for ( i=0; i<3; ++i) {
      ...
      dotProds[i] = vdot_c( scToXyz[(i+2)%3], pltCrossProds[i]);
    }
...
/**/
    ...
    double lox02pi = 3*M_PI;
      ...
      if ( (hix02pi-lox02pi) <= M_PI) { /* az's max del<=180 w/range 0 to 360 */
        ...
        if ( vdot_c( viewUpAbf, spudf->uvnorms+(3*ip)) <= 0.0) hiz = 1.1;
...
/**/
    ...
    long iPlus1;
      ...
      x = vnorm_c(axis);                       /* (perpendicular to axis) ... */
      if ( x != 0.0 && x != 1.0) {
        x = 1.0 / x;              /* (make axis unit vector so its horizontal */
        vscl_c(x,axis,axis);/* length is sin(alt) of max extent of great circle) */
      }
      if ( loz > -1.0) {        
      ...
      if ( hiz < 1.0) {
        vminus_c( axis, axis);                  /* (vhi X vlo) = -(vlo X vhi) */
...
#       endif
         ...
         */
        for ( j=0; j<3; ++j) {
          if ( (dotProds[j] * vdot_c(scToXyz[3],pltCrossProds[j])) < 0.0) {
        ...
        vsub_c( p, v[0], pltToP); /* plate-to-point vector for dot w/plt norm */
        if ( vdot_c( pltNorm, pltToP) > 0.0)     /* pt & s/c on same sides of */
...
#define MAX(A,B) ((A)>(B)?(A):(B))
  ...
  for ( ip=0; ip<numface; ++ip) {
     ...
     */
    if ( vdot_c( viewabf, spudf->uvnorms+(3*ip)) <= 0.0) continue;
